...
const EMPTY_DEBT = { name: "", balance: "", interestRate: "", minimumPayment: "" };
...
export function DebtPlanner({ debts, strategy, extraPayment, onAdd, onDelete, onSetStrategy, onSetExtra }: DebtPlannerProps) {
  ...
  // Build chart data — sample every N months to keep it readable
  const chartMonths = useMemo(() => {
    ...
    if (sampled[sampled.length - 1]?.month !== plan.months[plan.months.length - 1]?.month) {
      sampled.push(plan.months[plan.months.length - 1]);
    }
    return sampled;
  }, [plan]);
  ...
  const handleAdd = () => {
    ...
    if (!rate || rate < 0) { setFormError("Enter a valid interest rate"); return; }
    if (!min || min <= 0) { setFormError("Enter a minimum payment"); return; }
    setFormError("");
    ...
    <div className="debt-planner">
      ...
      <div className="debt-kpi-row">
        <div className="debt-kpi">
          <span className="debt-kpi-label">Total debt</span>
          <span className="debt-kpi-value danger">{fmt(totalBal)}</span>
        </div>
        <div className="debt-kpi">
          <span className="debt-kpi-label">Payoff date</span>
          <span className="debt-kpi-value">{plan.payoffDate || "—"}</span>
        </div>
        <div className="debt-kpi">
          <span className="debt-kpi-label">Total interest</span>
          <span className="debt-kpi-value danger">{fmt(plan.totalInterest)}</span>
        </div>
        <div className="debt-kpi success">
          <span className="debt-kpi-label">Interest saved vs min-only</span>
          <span className="debt-kpi-value success">{fmt(Math.max(0, saved.baseline - saved.avalanche))}</span>
        </div>
      </div>
        ...
        <div className="planner-left">
          ...
          <div className="strategy-card">
            ...
            <div className="strategy-toggle">
              <button
                ...
                onClick={() => onSetStrategy("avalanche")}
              >
              ...
              <button
                ...
                onClick={() => onSetStrategy("snowball")}
              >
                <span className="strategy-icon">⛄</span>
                <div>
                  <strong>Snowball</strong>
                  <p>Pay smallest balance first — builds momentum fastest</p>
                </div>
              </button>
            </div>
          </div>
          ...
          <div className="extra-card">
            ...
            <input
              ...
              step="25"
              value={extraPayment}
              onChange={(e) => onSetExtra(parseInt(e.target.value))}
            ...
            <div className="extra-impact">
              <div className="impact-row">
                <span>Minimum only ({fmt(minTotal)}/mo)</span>
                <span className="danger">{fmt(saved.baseline)} interest</span>
              </div>
              <div className="impact-row">
                <span>With extra payment</span>
                <span className="success">{fmt(strategy === "avalanche" ? saved.avalanche : saved.snowball)} interest</span>
              </div>
              <div className="impact-row bold">
                <span>You save</span>
                <span className="success">
                  {fmt(Math.max(0, saved.baseline - (strategy === "avalanche" ? saved.avalanche : saved.snowball)))}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="debt-list-card">
            ...
            {showForm && (
              <div className="debt-form">
                <input className="debt-input" placeholder="Debt name (e.g. Visa card)" value={form.name}
                  onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
                <div className="debt-form-row">
                  <div className="debt-field">
                    <label>Balance ($)</label>
                    <input className="debt-input" type="number" placeholder="5000" value={form.balance}
                      onChange={(e) => setForm((f) => ({ ...f, balance: e.target.value }))} />
                  </div>
                  <div className="debt-field">
                    <label>Interest rate (%)</label>
                    <input className="debt-input" type="number" placeholder="19.99" value={form.interestRate}
                      onChange={(e) => setForm((f) => ({ ...f, interestRate: e.target.value }))} />
                  </div>
                  <div className="debt-field">
                    <label>Min. payment ($)</label>
                    <input className="debt-input" type="number" placeholder="150" value={form.minimumPayment}
            ...
            {debts.length === 0 ? (
              <p className="empty-hint">Add your debts above to see your payoff plan.</p>
            ) : (
              <div className="debts">
                {debts.map((d) => (
                  <div key={d.id} className="debt-entry">
                    ...
                    <div className="debt-entry-numbers">
                      <div>
                        <span className="debt-entry-label">Balance</span>
                        <span className="debt-entry-val danger">{fmt(d.balance)}</span>
                      </div>
                      <div>
                        <span className="debt-entry-label">Min payment</span>
                        <span className="debt-entry-val">{fmt(d.minimumPayment)}</span>
                      </div>
                      <button
                        className={`del-btn ${confirmId === d.id ? "confirm" : ""}`}
                        onClick={() => handleDelete(d.id)}
                      >{confirmId === d.id ? "sure?" : "×"}</button>
                    </div>
                  </div>
                ))}
              </div>
                    ...
                    return (
                      <div key={m.month} className="chart-col">
                        <div className="chart-bar-wrap">
                          <div
                            className={`chart-bar ${isLast ? "last" : ""}`}
            ...
            {plan.monthsToPayoff > 0 && (
              <div className="timeline-milestones">
                <div className="milestone">
                  ...
                  <span>Debt-free in <strong>{plan.monthsToPayoff} months</strong> ({plan.payoffDate})</span>
                </div>
                <div className="milestone">
                  <span className="ms-dot interest" />
                  <span>Total interest paid: <strong className="danger">{fmt(plan.totalInterest)}</strong></span>
                </div>
              </div>
            )}
          </div>
