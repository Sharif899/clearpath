...
export function BudgetManager({ budgets, expenses, monthlyIncome, onUpdate, onSetIncome }: BudgetManagerProps) {
  ...
  const expectedPct   = (new Date().getDate() / daysInMonth) * 100;
    ...
    <div className="budget-manager">
      <div className="income-card">
        <div className="income-left">
          ...
          <p className="income-sub">Your take-home pay</p>
        </div>
        <div className="income-input-wrap">
          <span className="income-symbol">$</span>
          <input className="income-input" type="number"
            value={monthlyIncome}
      ...
      <div className="allocation-bar-section">
        ...
        {totalBudgeted > monthlyIncome && (
          <p className="alloc-warning">⚠ Budgets exceed income by {fmt(totalBudgeted - monthlyIncome)}</p>
        )}
      </div>
            ...
            <div key={h.category} className={`budget-card ${h.over ? "over" : ""}`}>
              ...
              <div className="budget-amounts">
                ...
                <div className="budget-limit-wrap">
                  ...
                  <input className="budget-limit-input" type="number" value={h.limit} min="0"
                    onChange={(e) => onUpdate(h.category, Math.max(0, parseFloat(e.target.value) || 0))} />
                </div>
              </div>

              <div className="budget-bar-track">
                <div className="budget-bar-expected" style={{ width: `${Math.min(expectedPct, 100)}%` }} />
              ...
              <div className="budget-footer">
                ...
                <span className={h.over ? "over" : ""}>
                  {h.over ? `${fmt(h.spent - h.limit)} over` : `${fmt(h.limit - h.spent)} left`}
                </span>
              </div>
            </div>
          );
