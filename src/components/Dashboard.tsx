...
import {
  totalSpent, monthlyExpenses, budgetHealth,
  totalDebt, fmt, fmtPct
} from "../lib/finance";
...
function BudgetRing({ pct }: { pct: number }) {
  ...
  return (
    <svg width="130" height="130" viewBox="0 0 130 130">
      ...
      />
      <text x="65" y="60" textAnchor="middle" fill="#f0f0f0" fontSize="20" fontWeight="700" fontFamily="Sora, sans-serif">
        {Math.round(pct)}%
      </text>
      <text x="65" y="78" textAnchor="middle" fill="#888" fontSize="11" fontFamily="Sora, sans-serif">
        of budget
      </text>
    </svg>
  );
...
export function Dashboard({ expenses, budgets, debts, monthlyIncome, onNavigate }: DashboardProps) {
  ...
  const netWorth = monthlyIncome - debt;
    ...
    <div className="dashboard">
      ...
      <div className="kpi-row">
        <div className="kpi-card accent">
          ...
          <span className="kpi-sub">set in settings</span>
        </div>
        ...
        <div className={`kpi-card ${netWorth >= 0 ? "success" : "danger"}`}>
          <span className="kpi-label">Cash vs debt</span>
          <span className="kpi-value">{fmt(Math.abs(monthlyIncome - debt))}</span>
          <span className="kpi-sub">{netWorth >= 0 ? "ahead" : "behind"}</span>
        </div>
      </div>
      ...
      <div className="dash-middle">
        <div className="dash-card ring-card">
          ...
          <div className="ring-stats">
            ...
            <div className="ring-stat">
              <span>{fmt(Math.max(0, totalBudget - spent))}</span>
              <span>remaining</span>
            </div>
          </div>
          {overBudget.length > 0 && (
            <div className="over-alert">
                ...
                <div key={h.category} className="cat-item">
                  ...
                  <div className="cat-item-info">
                    ...
                    <div className="cat-bar-track">
                      <div
                        className={`cat-bar-fill ${h.over ? "over" : ""}`}
                        style={{
        ...
        <div className="dash-card recent-card">
          ...
          {recentExpenses.length === 0 ? (
            ...
            <div className="recent-list">
              ...
              <button className="link-btn" onClick={() => onNavigate("expenses")}>
                View all expenses →
              </button>
            </div>
          )}
      ...
      {debts.length > 0 && (
        <div className="dash-card debt-snap">
          ...
          <div className="debt-list">
            {debts.map((d) => {
              const pct = (d.balance / totalDebt(debts)) * 100;
              return (
                <div key={d.id} className="debt-item">
                  <div className="debt-item-top">
                    <span className="debt-item-name">{d.name}</span>
                    <span className="debt-item-rate">{d.interestRate}% APR</span>
                    <span className="debt-item-bal">{fmt(d.balance)}</span>
                  </div>
                  <div className="cat-bar-track">
