...
export function AIInsight({ expenses, budgets, debts, monthlyIncome }: AIInsightProps) {
  ...
  const generate = async () => {
    ...
    const topCats     = [...health].sort((a, b) => b.spent - a.spent).slice(0, 3)
      .map((h) => `${CATEGORIES[h.category].label} (${fmt(h.spent)})`).join(", ");
    const debtSummary = debts.length > 0
      ? debts.map((d) => `${d.name}: ${fmt(d.balance)} at ${d.interestRate}%`).join("; ")
      : "No debts";

    const prompt = `My financial snapshot this month:
- Monthly income: ${fmt(monthlyIncome)}
- Total spent: ${fmt(spent)}
- Budget remaining: ${fmt(Math.max(0, budgets.reduce((s,b)=>s+b.limit,0) - spent))}
- Top spending: ${topCats}
- Over budget: ${overBudget.length > 0 ? overBudget.join(", ") : "None"}
- Total debt: ${fmt(debt)} (${debts.length} accounts)
- Debt details: ${debtSummary}

Give exactly 3 sharp, specific, actionable financial insights. Number them 1, 2, 3. Be direct and honest — no generic tips. Under 120 words total.`;
    ...
    try {
      ...
      setError("Could not load insight. Add VITE_ANTHROPIC_API_KEY to your environment variables.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="ai-insight">
      <div className="insight-header">
        <div>
          ...
          <p className="insight-sub">Claude analyses your expenses, budgets, and debts together</p>
        </div>
      ...
      {!insight && !loading && !error && (
        <div className="insight-empty">
          ...
          <p>{expenses.length > 0 || debts.length > 0
            ? "Get personalised insights based on your real financial data."
            : "Add some expenses or debts first, then come back for insights."}</p>
        </div>
      )}
      ...
      <button className="insight-btn" onClick={generate}
        disabled={loading || (expenses.length === 0 && debts.length === 0)}>
